...
.. note:: 
   **Hàm dịch vụ ngắt** 
   ...
   - Sau khi ISR hoàn thành, hệ thống chuyển sang “Helper thread” [1]_ dựa trên độ ưu tiên của Thread. Helper thread này có thể được kích hoạt bằng cách sử dụng một loại kernel object như FIFO, LIFO hoặc semaphore để xử lý các công việc liên quan đến ngắt.*
...
.. image:: ./images/k_is_ok.png
  :width: 100%
  :alt: Một số API Kernel cho phép ngắt đến Thread
...
.. note:: **Ngắt Thông thường được tạo ngay khi biên dịch chương trình**
.. .. WARNING::
...
.. Attention:: ``IRQ_CONNECT()`` bản thân nó không phải là một hàm C, không thao tác bên trong hàm. Vì vậy, tất cả đối số của nó cần phải được cung cấp ngay thời điểm gọi.
...
.. Tip:: Xem thêm: `Direct Interrupt <https://docs.zephyrproject.org/latest/kernel/services/interrupts.html#c.IRQ_DIRECT_CONNECT>`__
...
.. Note:: **Dựa trên các Ngắt Tổng quát trên, Zephyr cung cấp các API của các Ngắt Ứng dụng, được tạo nên bởi các hàm trên, bao gồm:** 
...
.. Note:: Xem: `GPIO Interrupt <./gpio_interrupt.html>`__

Ứng dụng - Ngắt Timer
---------------------

.. Note:: Xem: `Timer Interrupt <./timer_interrupt.html>`__

Ứng dụng - Ngắt UART
--------------------

.. Note:: Xem: `UART Interrupt <./uart_interrupt.html>`__
...
.. Attention:: Khóa IRQ là cụ thể cho từng Thread.
   ...
   Điều này đảm bảo Thread A sẽ không bị gián đoạn cho đến khi nó đã mở khóa IRQ của mình một cách rõ ràng.
...
.. Hint:: Vô hiệu hóa một IRQ, điều này ngăn tất cả các Thread trong hệ thống bị gián đoạn bởi ISR liên quan, không chỉ là Thread đã vô hiệu hóa IRQ.


Các API thường gặp
--------------------

.. Note:: Xem: `API Thường gặp <./command.html>`__



Thực hành
--------------------
.. Note:: Xem: `Thực hành <./exercise.html>`__


.. [1]
   *Là một Thread trong hệ thống mà được sử dụng để xử lý công việc bổ
